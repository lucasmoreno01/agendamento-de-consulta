...
function validateForm(form) {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(form.date))
    errors.date = "Informe a data no formato AAAA-MM-DD."
  if (!/^\d{2}:\d{2}$/.test(form.time))
    errors.time = "Informe o horario no formato HH:MM."
        ...
        [
          {
            text: "Ver detalhes",
            onPress: () =>
              router.replace(`/appointments/detail/${appointment.id}`),
          },
        ],
      )
    } catch (error) {
      ...
      <ScreenContainer>
        <View style={styles.section}>
          ...
          <View style={styles.summaryCard}>
            ...
            {form.notes.trim() ? (
              <SummaryLine label="Observacoes" value={form.notes.trim()} />
            ) : null}
          </View>
          <AppButton
    ...
    <ScreenContainer>
      <View style={styles.section}>
        ...
        <Text style={styles.description}>
          Preencha os dados para revisar e confirmar seu agendamento.
        </Text>
        ...
        {errors.specialty ? (
          <Text style={styles.fieldError}>{errors.specialty}</Text>
        ) : null}
        ...
        {form.specialty ? (
          <>
            ...
            {loadingError && professionals.length === 0 ? (
              <Text style={styles.fieldError}>{loadingError}</Text>
            ) : null}
            {errors.professional ? (
              <Text style={styles.fieldError}>{errors.professional}</Text>
            ) : null}
          </>
        ) : null}
